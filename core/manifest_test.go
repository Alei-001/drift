...
func TestSnapshotToManifest_ManifestToSnapshot_RoundTrip(t *testing.T) {
	...
	got := ManifestToSnapshot(m)
	if got == nil {
		t.Fatal("ManifestToSnapshot returned nil")
	}
	if got.ID != s.ID {
		t.Errorf("ID mismatch")
	}
	...
	if got.Files != nil {
		t.Errorf("Files: got %v, want nil", got.Files)
	}
