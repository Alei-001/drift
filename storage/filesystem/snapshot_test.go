...
func TestFS_ListSnapshots_NilFiles(t *testing.T) {
	...
	for _, s := range snaps {
		if s.Files != nil {
			t.Errorf("snapshot %x: Files should be nil, got %d entries", s.ID.Hash[:8], len(s.Files))
		}
	}
}
...
func TestFS_ListSnapshots_ManifestFallback(t *testing.T) {
	...
	if snaps2[0].Files != nil {
		t.Errorf("second listing should use manifest (nil Files), got %d entries", len(snaps2[0].Files))
	}
}
...
func TestFS_ListSnapshots_LargeCount(t *testing.T) {
	...
	for _, s := range snaps {
		if s.Files != nil {
			t.Errorf("Files should be nil (manifest used), got %d entries", len(s.Files))
		}
	}
