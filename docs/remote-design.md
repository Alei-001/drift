...
- `drift push` / `drift pull` 默认只同步当前分支：同步该分支链上的快照及其引用的 chunk，以及该分支的 ref。
- `--all` 参数同步整个仓库（所有分支、tag、快照、chunk）。
...
internal/remote/
    ...
    sync_batch.go        远程 chunk hash 批量列举
    sync_index.go        Pull 后 index 重建
    sync_remote_io.go    远程对象读写封装
    sync_scope.go        同步范围确定（全仓库/单分支）
    sync_transfer.go     Push/Pull 对象传输核心逻辑
    mock_remote_test.go  内存 mock RemoteFS（仅测试用）
    sync_test.go         同步逻辑测试
```

**协议注册采用 init 注册模式**（与 `internal/filetype/init.go` 引擎注册一致）：每个协议实现文件在 `init()` 中调用 `remote.Register(name, factory)`，sync 逻辑通过 `remote.NewRemoteFS(cfg)` 按名称查找已注册的 factory。新增协议 = 新增一个文件 + init 注册，无需修改 remote.go 的注册表或 sync 逻辑。
...
**扩展点**：新增协议只需 ① 实现 RemoteFS 接口 ② 在 init() 中 Register。sync 逻辑、注册表、cmd 层均无需修改。
...
```
cmd/
  remote.go          新增：remote add/remove/list/set-url/test/rename/show 命令
  remote_add.go      新增：交互式配置向导
  push.go            新增：push 命令
  pull.go            新增：pull 命令
  clone.go           新增：clone 命令（克隆远程仓库）
  ls_remote.go       新增：ls-remote 命令（列出远程引用）
  export.go          新增：export 命令（导出快照为 zip 归档）
  import.go          新增：import 命令（从其他分支导入文件）
internal/
  remote/            新增包（见 4.1）
  porcelain/
...
drift remote rename <old> <new>         重命名远程
drift remote show <name>                显示远程配置
```
...
```
drift push [remote-name] [--branch <b>] [--all] [--dry-run] [--quiet|-q] [--json]
drift pull [remote-name] [--branch <b>] [--all] [--dry-run] [--restore] [--quiet|-q] [--json]
```

**参数说明**：
- `remote-name`：可选，默认用配置中的第一个远程（若只有一个）或报错提示指定
- `--branch <b>`：只同步指定分支链（默认全仓库）
- `--all`：同步所有分支（默认只同步当前分支）
- `--dry-run`：只显示会同步什么，不实际执行
- `--restore`：（仅 pull）pull 后自动恢复工作区文件到最新分支 tip
- `--quiet`：成功时无输出
- `--json`：JSON 输出
...
- `RemoteFS` 接口的内存 mock 实现（`mock_remote_test.go`），用于测试 sync 逻辑：
...
1. **RemoteFS 接口 + 协议注册表 + mock 实现 + sync 逻辑**（不含真实协议）
   - `internal/remote/remote.go`（含 `ProtocolFactory` 注册表 + `Register`/`NewRemoteFS`）、sync 拆分文件（`sync_batch.go`、`sync_index.go`、`sync_remote_io.go`、`sync_scope.go`、`sync_transfer.go`）、`sync_test.go`
...
**扩展协议示例**：未来加 S3 时，只需新增 `internal/remote/s3.go`（实现 RemoteFS 接口 + init 注册），无需改 remote.go / sync 逻辑 / config.go / cmd 任何文件。
