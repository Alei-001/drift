...
func RunDaemonLoop(ctx context.Context, st *store.StoreSet, cwd string, interval int, keep int, cfg *core.CoreConfig) {
	if cfg == nil {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			os.Remove(pidPath)
			os.Remove(statePath)
			project.RemoveLockIfOwned(filepath.Join(driftDir, "workspace.lock"), os.Getpid())
		}
	}()
	...
	for {
		...
		case <-ticker.C:
			...
			// snapshot.DetectChanges acquires+releases the workspace lock; if another
			// operation holds it, detection simply fails and we retry next
			// tick. This replaces the former IsWorkspaceLocked pre-check.
			changes, err := snapshot.DetectChanges(ctx, st, cwd, cfg)
			if err != nil {
			...
			msg := gc.AutoSavePrefix + " " + time.Now().Format("2006-01-02 15:04")
			_, err = snapshot.CreateSnapshotInLock(ctx, st, cwd, msg, "drift", cfg, false)
			if err != nil {
				// errs.ErrNothingToSave is not a real error: the detect phase found
				...
				if errors.Is(err, errs.ErrNothingToSave) {
					project.ReleaseWorkspaceLock(cwd)
					continue
				}
			...
			if keep > 0 {
				pruned, pruneErr := pruneAutoSnapshots(ctx, st, keep)
				state.Pruned += pruned
				if pruneErr != nil {
...
func pruneAutoSnapshots(ctx context.Context, st *store.StoreSet, keep int) (int, error) {
	if keep <= 0 {
		return 0, nil
	}

	snapshots, err := st.Snapshots.ListSnapshots(ctx, nil)
	if err != nil {
	...
	reachable, err := gc.ComputeReachableSet(ctx, st)
	if err != nil {
	...
	for i := keep; i < len(autoSnaps); i++ {
		...
		if err := st.Snapshots.DeleteSnapshot(ctx, autoSnaps[i].ID); err != nil {
