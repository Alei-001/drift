...
func init() {
	remote.Register("webdav", NewWebDAVFS)
}

// WebDAVFS implements remote.RemoteFS over the WebDAV protocol using gowebdav.
...
// NewWebDAVFS constructs a WebDAVFS from a remote.RemoteConfig. The URL must be a
...
func NewWebDAVFS(cfg remote.RemoteConfig) (remote.RemoteFS, error) {
	if cfg.URL == "" {
...
// resolve joins a relative path with the root. Per the remote.RemoteFS path
...
func (w *WebDAVFS) Stat(ctx context.Context, p string) (*remote.RemoteInfo, error) {
...
func (w *WebDAVFS) Write(ctx context.Context, p string, r io.Reader) error {
	...
	if err := remote.EnsureRemoteDir(ctx, w, path.Dir(p)); err != nil {
		return err
	}
...
func (w *WebDAVFS) List(ctx context.Context, p string) ([]remote.RemoteInfo, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	resolved := w.resolve(p)
...
// Ensure WebDAVFS satisfies remote.RemoteFS at compile time.
var _ remote.RemoteFS = (*WebDAVFS)(nil)
