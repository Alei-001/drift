...
// snapshot's hash.
func gcPutSnapshot(st *store.StoreSet, tag byte, prevID *core.SnapshotID, files []core.FileEntry) core.Hash {
	snap := &core.Snapshot{
		Timestamp: int64(tag),
		PrevID:    prevID,
		Files:     files,
...
func TestCollectGarbage_AllReachable(t *testing.T) {
	...
	dir := t.TempDir()
	snapHash := gcPutSnapshot(st, 0x01, nil, nil)
	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snapHash})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})

	report, err := CollectGarbage(context.Background(), st, dir, false, 0)
	if err != nil {
...
func TestCollectGarbage_ReclaimsUnreachable(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, []core.FileEntry{{Path: "a", Chunks: []core.Hash{chunk1}}})
	snap2 := gcPutSnapshot(st, 0x02, &core.SnapshotID{Hash: snap1}, []core.FileEntry{{Path: "b", Chunks: []core.Hash{chunk2}}})
	snap3 := gcPutSnapshot(st, 0x03, nil, []core.FileEntry{{Path: "c", Chunks: []core.Hash{chunk3}}}) // orphan

	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snap2})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})

	st.Chunks.PutChunk(context.Background(), c1)
	st.Chunks.PutChunk(context.Background(), c2)
	st.Chunks.PutChunk(context.Background(), c3)
	...
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snap1}); err != nil {
		t.Errorf("snap1 should be preserved: %v", err)
	}
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snap2}); err != nil {
		t.Errorf("snap2 should be preserved: %v", err)
	}
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snap3}); err == nil {
		t.Error("snap3 should have been removed")
	}
	// chunk1 and chunk2 preserved; chunk3 gone.
	if ok, _ := st.Chunks.HasChunk(context.Background(), chunk1); !ok {
		t.Error("chunk1 should be preserved")
	}
	if ok, _ := st.Chunks.HasChunk(context.Background(), chunk2); !ok {
		t.Error("chunk2 should be preserved")
	}
	if ok, _ := st.Chunks.HasChunk(context.Background(), chunk3); ok {
...
func TestCollectGarbage_SharedChunkPreserved(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, []core.FileEntry{{Path: "a", Chunks: []core.Hash{chunkA}}}) // reachable via main
	snap3 := gcPutSnapshot(st, 0x03, nil, []core.FileEntry{{Path: "c", Chunks: []core.Hash{chunkA}}}) // orphan

	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snap1})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})
	st.Chunks.PutChunk(context.Background(), cA)
	...
	if ok, _ := st.Chunks.HasChunk(context.Background(), chunkA); !ok {
...
func TestCollectGarbage_DryRunNoDelete(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, []core.FileEntry{{Path: "a", Chunks: []core.Hash{chunk1}}}) // reachable
	snap2 := gcPutSnapshot(st, 0x02, nil, []core.FileEntry{{Path: "b", Chunks: []core.Hash{chunk2}}}) // orphan
	...
	report, err := CollectGarbage(context.Background(), st, dir, true, 0)
	if err != nil {
...
func TestCollectGarbage_PrevIDChain(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, nil)
	snap2 := gcPutSnapshot(st, 0x02, &core.SnapshotID{Hash: snap1}, nil)
	snap3 := gcPutSnapshot(st, 0x03, &core.SnapshotID{Hash: snap2}, nil)
	snap4 := gcPutSnapshot(st, 0x04, nil, nil) // orphan

	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snap3})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})
	...
	for _, h := range []core.Hash{snap1, snap2, snap3} {
		if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: h}); err != nil {
			t.Errorf("snapshot %x should be preserved: %v", h, err)
		}
	}
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snap4}); err == nil {
...
func TestCollectGarbage_TagKeepsSnapshot(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, nil) // reachable only via tag
	snap2 := gcPutSnapshot(st, 0x02, nil, nil) // reachable via main

	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snap2})
	st.Refs.SetRef(context.Background(), "tags/v1", &core.Reference{Name: "tags/v1", Type: core.RefTypeTag, Target: snap1})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})
...
func TestCollectGarbage_ZeroTargetSkipped(t *testing.T) {
	...
	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: core.Hash{}})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})
	st.Snapshots.PutSnapshot(context.Background(), &core.Snapshot{ID: core.SnapshotID{Hash: snapOrphan}, PrevID: nil})
	...
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snapOrphan}); err == nil {
...
func TestCountUnreachableSnapshots(t *testing.T) {
	...
	snap1 := gcPutSnapshot(st, 0x01, nil, nil) // reachable
	gcPutSnapshot(st, 0x02, nil, nil)          // orphan
	gcPutSnapshot(st, 0x03, nil, nil)          // orphan

	st.Refs.SetRef(context.Background(), "heads/main", &core.Reference{Name: "heads/main", Type: core.RefTypeBranch, Target: snap1})
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{Name: "HEAD", Type: core.RefTypeHead, SymRef: "heads/main"})

	count, err := CountUnreachableSnapshots(context.Background(), st, dir)
	if err != nil {
...
func TestCollectGarbage_DetachedHeadPreserved(t *testing.T) {
	...
	snapA := gcPutSnapshot(st, 0x01, nil, nil)
	snapB := gcPutSnapshot(st, 0x02, &core.SnapshotID{Hash: snapA}, nil)

	// Detached HEAD: SymRef is empty, Target points directly at B.
	st.Refs.SetRef(context.Background(), "HEAD", &core.Reference{
		Name:   "HEAD",
	...
	// B must still exist.
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snapB}); err != nil {
		t.Errorf("snapB should be preserved by detached HEAD: %v", err)
	}
	// A must also exist (reachable from B via PrevID).
	if _, err := st.Snapshots.GetSnapshot(context.Background(), core.SnapshotID{Hash: snapA}); err != nil {
