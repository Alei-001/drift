...
func collectRoots(ctx context.Context, st *store.StoreSet) ([]core.Hash, error) {
	var roots []core.Hash
	for _, prefix := range []string{"heads/", "tags/"} {
		refs, err := st.Refs.ListRefs(ctx, prefix)
		if err != nil {
...
func bfsReachable(ctx context.Context, st *store.StoreSet, withCache bool) (visited map[core.Hash]bool, snapCache map[core.Hash]*core.Snapshot, err error) {
	roots, rerr := collectRoots(ctx, st)
	if rerr != nil {
		return nil, nil, rerr
	}
	...
	for len(queue) > 0 {
		...
		snap, gerr := st.Snapshots.GetSnapshot(ctx, core.SnapshotID{Hash: h})
		if gerr != nil {
...
func ComputeReachableSet(ctx context.Context, st *store.StoreSet) (map[core.Hash]bool, error) {
	visited, _, err := bfsReachable(ctx, st, false)
	return visited, err
}
...
func computeReachability(ctx context.Context, st *store.StoreSet) (map[core.Hash]bool, []*core.SnapshotSummary, map[core.Hash]*core.Snapshot, error) {
	visited, snapCache, err := bfsReachable(ctx, st, true)
	if err != nil {
		return nil, nil, nil, err
	}
	allSnapshots, err := st.Snapshots.ListSnapshots(ctx, nil)
	if err != nil {
...
// serializes GC against save/switch/rest, which are the only operations
// that add new chunks or snapshots.
func CollectGarbage(ctx context.Context, st *store.StoreSet, workDir string, dryRun bool, keepAuto int) (GCReport, error) {
	...
	reachable, allSnapshots, snapCache, err := computeReachability(ctx, st)
	if err != nil {
		return report, err
	}
	...
	for _, snap := range unreachable {
		...
		if !dryRun {
			if err := st.Snapshots.DeleteSnapshot(ctx, snap.ID); err != nil {
	...
	for _, snap := range allSnapshots {
		...
		if cached, ok := snapCache[snap.ID.Hash]; ok {
			...
			f, err := st.Snapshots.GetSnapshot(ctx, snap.ID)
			if err != nil {
	...
	if st.Compactor != nil {
		cr, err := st.Compactor.CompactChunks(ctx, reachableChunks, dryRun)
		if err != nil {
		...
		allChunks, err := st.Chunks.ListChunks(ctx)
		if err != nil {
		...
		for _, ch := range allChunks {
			...
			if chunk, gerr := st.Chunks.GetChunk(ctx, ch); gerr == nil {
				report.FreedBytes += int64(chunk.Size)
			}
			report.ChunksRemoved++
			if !dryRun {
				if err := st.Chunks.DeleteChunk(ctx, ch); err != nil {
...
func CountUnreachableSnapshots(ctx context.Context, st *store.StoreSet, workDir string) (int, error) {
	if err := project.AcquireWorkspaceLock(workDir); err != nil {
	...
	reachable, err := ComputeReachableSet(ctx, st)
	if err != nil {
		return 0, err
	}
	allSnapshots, err := st.Snapshots.ListSnapshots(ctx, nil)
