...
func resolveBranchOrDefault(ctx context.Context, st *store.StoreSet, branchName string, all bool) string {
	if all || branchName != "" {
		return branchName
	}
	return branch.ResolveCurrentBranchName(ctx, st)
}
...
// NewRemoteFS cannot bypass it by forgetting to call remote.IsInsecureScheme.
...
// workspace-modifying operations (snapshot, switch, rest) do not observe
// an inconsistent local state while push is reading from it.
func PushToRemote(ctx context.Context, st *store.StoreSet, workDir, remoteName, branch string, all bool) (*PushResult, error) {
	...
	branch = resolveBranchOrDefault(ctx, st, branch, all)
	...
	stats, err := remote.Push(ctx, st, rfs, branch, opts)
	if err != nil {
...
func PushDryRun(ctx context.Context, st *store.StoreSet, workDir, remoteName, branch string, all bool) (*remote.SyncStats, error) {
	cfg, err := resolveRemoteConfigWithWarn(workDir, remoteName)
	if err != nil {
		return nil, err
	}
	...
	opts := remote.SyncOptions{Concurrency: parseConcurrency(cfg)}
	stats, err := remote.PushDryRun(ctx, st, rfs, branch, opts)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", remote.ErrNetwork, err)
	}
	return stats, nil
...
func PullFromRemote(ctx context.Context, st *store.StoreSet, workDir, remoteName, branch string, all bool) (*PullResult, error) {
	cfg, err := resolveRemoteConfigWithWarn(workDir, remoteName)
	if err != nil {
		return nil, err
	}
	...
	opts := remote.SyncOptions{Concurrency: parseConcurrency(cfg)}
	stats, err := remote.Pull(ctx, st, rfs, branch, opts)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", remote.ErrNetwork, err)
	}
...
// the workspace lock so the dry-run reflects a consistent local state.
func PullDryRun(ctx context.Context, st *store.StoreSet, workDir, remoteName, branch string, all bool) (*remote.SyncStats, error) {
	cfg, err := resolveRemoteConfigWithWarn(workDir, remoteName)
	if err != nil {
		return nil, err
	}
	...
	opts := remote.SyncOptions{Concurrency: parseConcurrency(cfg)}
	stats, err := remote.PullDryRun(ctx, st, rfs, branch, opts)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", remote.ErrNetwork, err)
	}
	return stats, nil
