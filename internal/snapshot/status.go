...
func DetectChanges(ctx context.Context, st *store.StoreSet, workDir string, cfg *core.CoreConfig) (*ChangeSummary, error) {
	...
	return DetectChangesNoLock(ctx, st, workDir, cfg)
}
...
func DetectChangesNoLock(ctx context.Context, st *store.StoreSet, workDir string, cfg *core.CoreConfig) (*ChangeSummary, error) {
	if cfg == nil {
		cfg = &core.DefaultConfig().Core
	}

	index, err := st.Index.GetIndex(ctx)
	if err != nil {
