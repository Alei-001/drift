...
// fileTask represents a changed file that needs chunking and store.
...
func processFileTask(ctx context.Context, st *store.StoreSet, task fileTask) (core.FileEntry, error) {
	...
	engine := engine.DetectEngine(task.relPath, header)
	if engine == nil {
	...
	err = chunkFile(ctx, task.wf.path, file, engine, task.wf.info.Size(), func(c *core.Chunk) error {
		...
		has, err := st.Chunks.HasChunk(ctx, c.Hash)
		if err != nil {
			return fmt.Errorf("check chunk existence %s: %w", c.Hash.String(), err)
		}
		if !has {
			if err := st.Chunks.PutChunk(ctx, c); err != nil {
...
func chunkFilesConcurrent(ctx context.Context, st *store.StoreSet, tasks []fileTask, bar *progressbar.ProgressBar) (map[string]core.FileEntry, error) {
	...
	for _, task := range tasks {
		...
		go func(t fileTask) {
			...
			entry, err := processFileTask(ctx, st, t)
			if err != nil {
				mu.Lock()
				if firstErr == nil {
					firstErr = err
