...
//
// Returns the imported FileEntry and an error. errs.ErrBranchNotFound is
// returned when the branch does not exist; errs.ErrSnapshotNotFound when the
// branch has no snapshots; errs.ErrFileNotFound when the file is not present
// in the branch's HEAD snapshot.
func ImportFileFromBranch(ctx context.Context, st *store.StoreSet, workDir, branchName, filePath string, cfg *core.CoreConfig) (*core.FileEntry, error) {
	_ = cfg
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return nil, fmt.Errorf("branch %q: %w", branchName, errs.ErrBranchNotFound)
		}
		return nil, fmt.Errorf("read branch ref: %w", err)
	}
	if ref.Target.IsZero() {
		return nil, fmt.Errorf("branch %q has no snapshots: %w", branchName, errs.ErrSnapshotNotFound)
	}
	...
	if targetEntry == nil {
		return nil, fmt.Errorf("%w: %q in branch %q", errs.ErrFileNotFound, relPath, branchName)
	}
	...
	if err := writeFileFromChunks(ctx, st, safePath, targetEntry.Chunks, perm); err != nil {
