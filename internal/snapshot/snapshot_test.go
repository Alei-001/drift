...
func TestCreateSnapshot_FirstCommit(t *testing.T) {
	...
	ms.SetIndex(context.Background(), &core.Index{
	...
	stored, err := store.Snapshots.GetSnapshot(context.Background(), snap.ID)
	if err != nil {
...
func TestCreateSnapshot_SecondCommit(t *testing.T) {
	...
	_, err = store.Snapshots.GetSnapshot(context.Background(), snap1.ID)
	if err != nil {
		t.Fatalf("GetSnapshot for first snapshot failed: %v", err)
	}
	_, err = store.Snapshots.GetSnapshot(context.Background(), snap2.ID)
	if err != nil {
...
func TestCreateSnapshot_NothingChanged(t *testing.T) {
	...
	if !errors.Is(err, errs.ErrNothingToSave) {
		t.Errorf("expected errs.ErrNothingToSave, got '%s'", err.Error())
	}
}
