...
// renders the result.
func ComputeStatSnapshots(ctx context.Context, st *store.StoreSet, snap1, snap2 *core.Snapshot) ([]FileStat, error) {
	...
	for i := range snap2.Files {
		...
		if !exists {
			ins, del, bin := computeSnapFileStat(ctx, st, nil, e2)
		...
		if e1.Size != e2.Size || !slices.Equal(e1.Chunks, e2.Chunks) {
			ins, del, bin := computeSnapFileStat(ctx, st, e1, e2)
	...
	for path, e1 := range snap1Files {
		...
		ins, del, bin := computeSnapFileStat(ctx, st, e1, nil)
...
func ComputeStatWorkspace(ctx context.Context, st *store.StoreSet, workDir string, cfg *core.CoreConfig, snap *core.Snapshot) ([]FileStat, error) {
	...
	walkErr := fsutil.Walk(workDir, cfg.IgnoreFile, func(path string, info os.FileInfo) error {
		...
		if !exists {
			...
			ins, del, bin := computeWorkFileStat(ctx, st, path, e1)
...
func computeSnapFileStat(ctx context.Context, st *store.StoreSet, e1, e2 *core.FileEntry) (ins, del int, isBinary bool) {
	...
	var c1, c2 []byte
	if e1 != nil {
		data, err := readAllChunks(ctx, st, e1.Chunks)
		if err != nil {
			return 0, 0, true
		}
		c1 = data
	...
	if e2 != nil {
		data, err := readAllChunks(ctx, st, e2.Chunks)
		if err != nil {
			return 0, 0, true
		}
		c2 = data
	...
	engine := engine.DetectEngine(path, header)
...
// or files larger than statLargeFileThreshold (to avoid OOM).
func computeWorkFileStat(ctx context.Context, st *store.StoreSet, workPath string, e1 *core.FileEntry) (ins, del int, isBinary bool) {
	...
	c1, err := readAllChunks(ctx, st, e1.Chunks)
	if err != nil {
		return 0, 0, true
	}
	...
	engine := engine.DetectEngine(e1.Path, header)
	if engine == nil || engine.Name() != "text" {
		return 0, 0, true
	}
...
// diffs.
func readAllChunks(ctx context.Context, st *store.StoreSet, hashes []core.Hash) ([]byte, error) {
	return io.ReadAll(stream.NewChunkReader(ctx, st.Chunks, hashes))
}
