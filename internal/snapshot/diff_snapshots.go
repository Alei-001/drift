...
func DiffFileInSnapshots(ctx context.Context, st *store.StoreSet, workDir string, snap1, snap2 *core.Snapshot, filePath string) ContentDiffResult {
	...
	reader1 := stream.NewChunkReader(ctx, st.Chunks, entry1.Chunks)
	reader2 := stream.NewChunkReader(ctx, st.Chunks, entry2.Chunks)
	...
	engine := engine.DetectEngine(filePath, header2)
