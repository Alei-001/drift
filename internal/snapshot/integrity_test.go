...
func integrityTestStore(t *testing.T) (*store.StoreSet, core.SnapshotID, core.Hash, core.Hash) {
	t.Helper()
	ms := memory.NewMemoryStorage()

	c1 := gcChunk([]byte("chunk-one-data"), 14)
	c2 := gcChunk([]byte("chunk-two-data"), 14)
	ms.PutChunk(context.Background(), c1)
	ms.PutChunk(context.Background(), c2)
	...
	return store.NewStoreSet(ms), snap.ID, c1.Hash, c2.Hash
}
...
func TestVerifyIntegrity_MissingChunk(t *testing.T) {
	...
	if err := store.Chunks.DeleteChunk(context.Background(), ch1); err != nil {
