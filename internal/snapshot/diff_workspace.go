...
func DiffWorkspaceFileVsSnapshot(ctx context.Context, st *store.StoreSet, workDir string, snapshot *core.Snapshot, filePath string) (ContentDiffResult, error) {
	...
	if info.Size() == snapEntry.Size {
		...
		snapHash, err := stream.HashChunkData(ctx, st.Chunks, snapEntry.Chunks)
		if err != nil {
	...
	engine := engine.DetectEngine(filePath, header)

	snapReader := stream.NewChunkReader(ctx, st.Chunks, snapEntry.Chunks)
