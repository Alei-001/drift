...
// failingHeadRefStorer wraps a ReferenceStorer and forces SetRef to fail only for
// the "HEAD" ref. All other SetRef calls (branch refs, etc.) and all other
// methods delegate to the embedded ReferenceStorer. Used to exercise the
...
type failingHeadRefStorer struct {
	store.ReferenceStorer
}

func (s *failingHeadRefStorer) SetRef(ctx context.Context, name string, ref *core.Reference) error {
	if name == "HEAD" {
		return errors.New("simulated HEAD SetRef failure")
	}
	return s.ReferenceStorer.SetRef(ctx, name, ref)
}

func makeFailingHeadSetRefStore(real *store.StoreSet) *store.StoreSet {
	return &store.StoreSet{
		Chunks:    real.Chunks,
		Snapshots: real.Snapshots,
		Refs:      &failingHeadRefStorer{ReferenceStorer: real.Refs},
		Index:     real.Index,
		Config:    real.Config,
...
// store set and both snapshot IDs.
func setupRestoreSnapshots(t *testing.T, dir string) (*store.StoreSet, core.SnapshotID, core.SnapshotID) {
	t.Helper()
...
func TestRestoreSnapshot_HeadUpdateFailureRollsBackBranch(t *testing.T) {
	...
	branchAfter, err := store.Refs.GetRef(ctx, "heads/main")
	if err != nil {
