...
func chunkFile(ctx context.Context, path string, r io.Reader, engine engine.Engine, fileSize int64, fn func(*core.Chunk) error) error {
	if engine == nil {
...
func CountFileLines(ctx context.Context, st *store.StoreSet, entry core.FileEntry) int {
	count, _ := CountFileLinesWithError(ctx, st, entry)
	return count
}
...
func CountFileLinesWithError(ctx context.Context, st *store.StoreSet, entry core.FileEntry) (int, error) {
	count := 0
