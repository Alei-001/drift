...
// (empty when no backup was needed, e.g. errs.ErrNothingToSave). cfg may be nil
...
func RestoreSnapshot(ctx context.Context, st *store.StoreSet, workDir string, snapshotID core.SnapshotID, filePath string, noBackup bool, cfg *core.CoreConfig) (backupID string, err error) {
	if cfg == nil {
	...
	snap, err := st.Snapshots.GetSnapshot(ctx, snapshotID)
	if err != nil {
	...
	if !noBackup {
		...
		backupSnap, backupErr := CreateSnapshotInLock(ctx, st, workDir, backupMsg, "drift", cfg, false)
		if backupErr != nil {
			if !errors.Is(backupErr, errs.ErrNothingToSave) {
	...
	if filePath == "" {
		if err = RestoreFilesToWorkspace(ctx, st, workDir, cfg.IgnoreFile, snap); err != nil {
		...
		headRef, headErr := st.Refs.GetRef(ctx, "HEAD")
		if headErr != nil {
		...
		if headRef.SymRef != "" {
			...
			oldBranchRef, oldBranchErr := st.Refs.GetRef(ctx, headRef.SymRef)
			if oldBranchErr != nil {
			...
			if err = st.Refs.SetRef(ctx, "HEAD", headRef); err != nil {
				...
				_ = st.Refs.SetRef(ctx, headRef.SymRef, oldBranchRef)
		...
		for _, entry := range snap.Files {
			...
			if err := writeFileFromChunks(ctx, st, fullPath, entry.Chunks, perm); err != nil {
		...
		if restoredEntry == nil {
			return backupID, fmt.Errorf("%w: %q in snapshot %s", errs.ErrFileNotFound, filePath, snapshotID.Hash.String())
		}

		if restoredEntry != nil {
			existingIndex, err := st.Index.GetIndex(ctx)
			if err != nil {
			...
			if err := st.Index.SetIndex(ctx, existingIndex); err != nil {
