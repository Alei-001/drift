...
// This matches the documented contract of *store.StoreSet.GetRef and the
...
func ResolveHeadSnapshot(ctx context.Context, st *store.StoreSet) *core.Snapshot {
...
func SnapshotFileDiff(ctx context.Context, st *store.StoreSet, snapshot *core.Snapshot) (added []core.FileEntry, modified []core.FileEntry, deleted []string, err error) {
	...
	if snapshot.PrevID != nil {
		prevSnap, err := st.Snapshots.GetSnapshot(ctx, *snapshot.PrevID)
		if err != nil {
...
func CountSnapshotChanges(ctx context.Context, st *store.StoreSet, summary *core.SnapshotSummary) (added, modified, deleted int) {
	snapshot, err := st.Snapshots.GetSnapshot(ctx, summary.ID)
	if err != nil {
		slog.Warn("load snapshot for changes", "snapshot", summary.ShortID(), "error", err)
		return 0, 0, 0
	}
	a, m, d, err := SnapshotFileDiff(ctx, st, snapshot)
	if err != nil {
...
func UndoLastSave(ctx context.Context, st *store.StoreSet, workDir string, cfg *core.CoreConfig) error {
	if cfg == nil {
	...
	if currentHash.IsZero() {
		return errs.ErrCannotUndo
	}

	currentSnap, err := st.Snapshots.GetSnapshot(ctx, core.SnapshotID{Hash: currentHash})
	if err != nil {
	...
	summary, err := DetectChangesNoLock(ctx, st, workDir, cfg)
	if err != nil {
	...
	if headRef.SymRef != "" {
		branchRef, err := st.Refs.GetRef(ctx, headRef.SymRef)
		if err != nil {
			return fmt.Errorf("read branch ref: %w", err)
		}
		branchRef.Target = prevHash
		if err := st.Refs.SetRef(ctx, headRef.SymRef, branchRef); err != nil {
	...
	if err := RebuildIndexFromSnapshot(ctx, st, core.SnapshotID{Hash: prevHash}); err != nil {
