...
func VerifyIntegrity(ctx context.Context, st *store.StoreSet, workDir, filter string, verbose bool) (*IntegrityReport, error) {
	...
	for _, snap := range snapshots {
		...
		full, err := st.Snapshots.GetSnapshot(ctx, snap.ID)
		if err != nil {
			snapshotCorrupt++
			continue
		}
	...
	for hash := range hashSet {
		...
		has, err := st.Chunks.HasChunk(ctx, hash)
		if err != nil {
			report.Corrupt++
		...
		chunk, err := st.Chunks.GetChunk(ctx, hash)
		if err != nil {
			report.Corrupt++
			results[hash] = ChunkCorrupt
			continue
