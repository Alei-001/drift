...
// empty; cfg may be nil (core.DefaultConfig is used). Returns errs.ErrNothingToSave
...
func CreateSnapshot(ctx context.Context, st *store.StoreSet, workDir string, message string, author string, cfg *core.CoreConfig) (*core.Snapshot, error) {
	...
	return CreateSnapshotInLock(ctx, st, workDir, message, author, cfg, true)
}

func CreateSnapshotInLock(ctx context.Context, st *store.StoreSet, workDir string, message string, author string, cfg *core.CoreConfig, showProgress bool) (*core.Snapshot, error) {
	if message == "" {
	...
	oldIndex, err := st.Index.GetIndex(ctx)
	if err != nil {
	...
	for _, f := range workspaceFiles {
		...
		if touchCounter%500 == 0 {
			if terr := project.TouchWorkspaceLock(workDir); terr != nil {
				return nil, terr
			}
		}
	...
	processedEntries, err := chunkFilesConcurrent(ctx, st, tasks, bar)
	if err != nil {
		return nil, err
	}
	...
	if !changed {
		return nil, errs.ErrNothingToSave
	}
	...
	if err := st.Refs.SetRef(ctx, symRef, branchRef); err != nil {
		return nil, fmt.Errorf("update branch: %w", err)
	}
	if err := st.Index.SetIndex(ctx, newIndex); err != nil {
