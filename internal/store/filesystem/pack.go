...
func (fs *FSStorage) loadPackIndex(name string) (*packIndex, error) {
	...
	if len(data) < packIndexHeaderSize {
		return nil, fmt.Errorf("pack index %s: file too short: %w", name, store.ErrCorrupted)
	}

	if string(data[:4]) != packIndexMagic {
		return nil, fmt.Errorf("pack index %s: bad magic: %w", name, store.ErrCorrupted)
	}
	if data[4] != packIndexVersion {
		return nil, fmt.Errorf("pack index %s: unsupported version %d: %w", name, data[4], store.ErrCorrupted)
	}

	count := binary.BigEndian.Uint32(data[5:9])
	if count > maxPackIndexEntries {
		return nil, fmt.Errorf("pack index %s: entry count %d exceeds max %d: %w", name, count, maxPackIndexEntries, store.ErrCorrupted)
	}
	...
	if int64(len(data)) < expectedSize {
		return nil, fmt.Errorf("pack index %s: file too short for %d entries: %w", name, count, store.ErrCorrupted)
	}
	...
	for i := uint32(0); i < count; i++ {
		...
		if _, exists := entries[hash]; exists {
			return nil, fmt.Errorf("pack index %s: duplicate hash %s: %w", name, hash.FullString(), store.ErrCorrupted)
		}
...
func (fs *FSStorage) readChunkFromPack(name string, entry packEntry, hash core.Hash) (*core.Chunk, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("stat pack %s: pack file missing while index exists: %w", name, store.ErrCorrupted)
		}
		return nil, fmt.Errorf("stat pack %s: %w", name, mapOSError(err))
	}
	packFileSize := stat.Size()
	if entry.length > maxPackEntryLength {
		return nil, fmt.Errorf("pack %s: entry length %d exceeds max %d: %w", name, entry.length, maxPackEntryLength, store.ErrCorrupted)
	}
	...
	if entry.length < store.ChunkHeaderSize {
		return nil, fmt.Errorf("pack %s: entry length %d below header size %d: %w", name, entry.length, store.ChunkHeaderSize, store.ErrCorrupted)
	}
	if entry.offset < 0 || entry.offset+int64(entry.length) > packFileSize {
		return nil, fmt.Errorf("pack %s: entry offset %d length %d exceeds pack size %d: %w", name, entry.offset, entry.length, packFileSize, store.ErrCorrupted)
	}

	f, err := os.Open(packPath)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("open pack %s: pack file missing while index exists: %w", name, store.ErrCorrupted)
		}
	...
	if _, err := io.ReadFull(f, buf); err != nil {
		return nil, fmt.Errorf("read pack %s: %w", name, store.ErrCorrupted)
	}

	header := buf[0]
	payload := buf[1:]
	compressed := header&store.ChunkFlagCompressed != 0

	var data []byte
	if compressed {
		decoded, err := fs.decompressFromReader(bytes.NewReader(payload))
		if err != nil {
			return nil, fmt.Errorf("decode chunk from pack %s: %w", name, store.ErrCorrupted)
		}
		data = decoded
	} else {
		data = payload
	...
	if computedHash != hash {
		return nil, fmt.Errorf("pack chunk %x integrity check failed: expected %s, got %s: %w", hash[:8], hash.FullString(), computedHash.FullString(), store.ErrCorrupted)
	}
