...
// instead returns an error wrapping store.ErrInvalidRef.
func TestGetRef_SymRefSelfReference(t *testing.T) {
	...
	if !errors.Is(err, store.ErrInvalidRef) {
		t.Errorf("GetRef(HEAD) error should wrap store.ErrInvalidRef, got: %v", err)
	}
}

// TestGetRef_SymRefCycle verifies that a cycle of symbolic references
// (A -> B -> A) is detected and reported as store.ErrInvalidRef rather
...
func TestGetRef_SymRefCycle(t *testing.T) {
	...
	if !errors.Is(err, store.ErrInvalidRef) {
		t.Errorf("GetRef(heads/A) error should wrap store.ErrInvalidRef, got: %v", err)
	}
}
