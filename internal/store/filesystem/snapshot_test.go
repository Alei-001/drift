...
func TestFS_ListSnapshots_LargeCount(t *testing.T) {
	...
	snaps, err := store.ListSnapshots(ctx, &store.ListOptions{Limit: 10, Offset: 0})
	if err != nil {
		t.Fatalf("ListSnapshots: %v", err)
	}
	if len(snaps) != 10 {
...
func TestFS_ListSnapshots_Pagination(t *testing.T) {
	...
	// Limit = 3
	snaps, err := store.ListSnapshots(ctx, &store.ListOptions{Limit: 3})
	if err != nil {
		t.Fatalf("ListSnapshots: %v", err)
	}
	if len(snaps) != 3 {
	...
	snaps, err = store.ListSnapshots(ctx, &store.ListOptions{Offset: 8})
	if err != nil {
	...
	// Offset beyond range
	snaps, err = store.ListSnapshots(ctx, &store.ListOptions{Offset: 100})
	if err != nil {
