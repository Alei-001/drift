...
func TestListSnapshots_Pagination(t *testing.T) {
	...
	// Limit only.
	page, _ := store.ListSnapshots(ctx, &store.ListOptions{Limit: 3})
	if len(page) != 3 {
		t.Errorf("Limit=3: got %d, want 3", len(page))
	}

	// Offset only.
	tail, _ := store.ListSnapshots(ctx, &store.ListOptions{Offset: 8})
	if len(tail) != 2 {
		t.Errorf("Offset=8: got %d, want 2", len(tail))
	}

	// Offset beyond length returns nil.
	empty, _ := store.ListSnapshots(ctx, &store.ListOptions{Offset: 100})
	if empty != nil {
		t.Errorf("Offset=100: got %v, want nil", empty)
	}

	// Limit + Offset.
	mid, _ := store.ListSnapshots(ctx, &store.ListOptions{Limit: 3, Offset: 3})
	if len(mid) != 3 {
