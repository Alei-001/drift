...
// store.ErrCorrupted so callers can handle it uniformly.
func (ms *MemoryStorage) GetSnapshot(ctx context.Context, id core.SnapshotID) (*core.Snapshot, error) {
	...
	if !ok {
		return nil, fmt.Errorf("get snapshot %s: %w", id.Hash.FullString(), store.ErrNotFound)
	}
	clone := store.CloneSnapshot(v)
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshal snapshot %s: %w", id.Hash.FullString(), store.ErrCorrupted)
	}
	if core.Hash(blake3.Sum256(recomputed)) != id.Hash {
		return nil, fmt.Errorf("snapshot %s integrity check failed: %w", id.Hash.FullString(), store.ErrCorrupted)
	}
	return clone, nil
}
...
func (ms *MemoryStorage) PutSnapshot(ctx context.Context, snapshot *core.Snapshot) error {
	...
	ms.snapshots[key] = store.CloneSnapshot(snapshot)
...
func (ms *MemoryStorage) ListSnapshots(ctx context.Context, opts *store.ListOptions) ([]*core.SnapshotSummary, error) {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
