...
func Validate(name string) error {
	if name == "" {
		return fmt.Errorf("ref name is empty: %w", store.ErrInvalidRef)
	}
	if name == "HEAD" {
		return nil
	}
	for _, c := range name {
		if c < 0x20 || c == 0x7F {
			return fmt.Errorf("invalid ref name: %q contains control character: %w", name, store.ErrInvalidRef)
		}
		if c == ' ' {
			return fmt.Errorf("invalid ref name: %q contains space: %w", name, store.ErrInvalidRef)
		}
		if c == '\\' || c == ':' {
			return fmt.Errorf("invalid ref name: %q contains %q: %w", name, string(c), store.ErrInvalidRef)
		}
	}
	if strings.HasPrefix(name, "-") || strings.HasPrefix(name, "/") {
		return fmt.Errorf("invalid ref name: %q starts with '-' or '/': %w", name, store.ErrInvalidRef)
	}
	if strings.Contains(name, "..") {
		return fmt.Errorf("invalid ref name: %q contains '..': %w", name, store.ErrInvalidRef)
	}
	base := strings.ToLower(filepath.Base(name))
	if IsWindowsReservedName(base) {
		return fmt.Errorf("invalid ref name: %q is a reserved name: %w", name, store.ErrInvalidRef)
	}
	...
	if reservedBareNames[base] {
		return fmt.Errorf("invalid ref name: %q is a reserved keyword: %w", name, store.ErrInvalidRef)
	}
	return nil
}
