...
func TestOpenProjectWithFactory_UsesProvidedStore(t *testing.T) {
	...
	memFactory := func(driftPath string) (*store.StoreSet, error) {
		factoryCalled = true
		return store.NewStoreSet(memory.NewMemoryStorage()), nil
	}
	...
	if _, ok := store.(*store.StoreSet); !ok {
		t.Fatalf("expected *store.StoreSet, got %T", store)
	}
}
...
func TestOpenProject(t *testing.T) {
	...
	ref, err := store.Refs.GetRef(context.Background(), "HEAD")
	if err != nil {
