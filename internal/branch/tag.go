...
func ListTags(ctx context.Context, st *store.StoreSet) ([]TagInfo, error) {
	...
	for _, ref := range refs {
		...
		if !ref.Target.IsZero() {
			if snap, snapErr := st.Snapshots.GetSnapshot(ctx, core.SnapshotID{Hash: ref.Target}); snapErr == nil {
...
func AddTag(ctx context.Context, st *store.StoreSet, cwd string, name string, snapID core.SnapshotID) error {
	...
	if _, err := st.Snapshots.GetSnapshot(ctx, snapID); err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return fmt.Errorf("snapshot '%s' not found: %w", snapID.Hash.String(), errs.ErrSnapshotNotFound)
		}
		return fmt.Errorf("check snapshot existence: %w", err)
	}

	refName := "tags/" + name
	if existing, err := st.Refs.GetRef(ctx, refName); err == nil && existing != nil {
		return fmt.Errorf("tag '%s' already exists: %w", name, errs.ErrTagAlreadyExists)
	...
	if err := st.Refs.SetRef(ctx, refName, ref); err != nil {
		return fmt.Errorf("set tag ref: %w", err)
	}
	return nil
}

// DeleteTag removes a tag by name. It returns errs.ErrTagNotFound if the tag
...
// same name (TOCTOU).
func DeleteTag(ctx context.Context, st *store.StoreSet, cwd string, name string) error {
	name = normalizeTagName(name)
	if name == "" {
		return fmt.Errorf("tag name is required")
	}
	...
	refName := "tags/" + name
	if _, err := st.Refs.GetRef(ctx, refName); err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return fmt.Errorf("tag '%s' not found: %w", name, errs.ErrTagNotFound)
		}
		return fmt.Errorf("check tag existence: %w", err)
	}
	if err := st.Refs.DeleteRef(ctx, refName); err != nil {
...
func RenameTag(ctx context.Context, st *store.StoreSet, cwd string, oldName, newName string) error {
	...
	oldRef, err := st.Refs.GetRef(ctx, oldRefName)
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return fmt.Errorf("tag '%s' not found: %w", oldName, errs.ErrTagNotFound)
		}
		return fmt.Errorf("check tag existence: %w", err)
	}
	if oldName == newName {
	...
	if _, err := st.Refs.GetRef(ctx, newRefName); err == nil {
		return fmt.Errorf("tag '%s' already exists: %w", newName, errs.ErrTagAlreadyExists)
	...
	if err := st.Refs.SetRef(ctx, newRefName, newRef); err != nil {
		return fmt.Errorf("create renamed tag: %w", err)
	}
	if err := st.Refs.DeleteRef(ctx, oldRefName); err != nil {
