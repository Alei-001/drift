...
func putTestSnapshot(t *testing.T, st *store.StoreSet) core.SnapshotID {
	t.Helper()
	snap := &core.Snapshot{
	...
	if err := st.Snapshots.PutSnapshot(context.Background(), snap); err != nil {
...
func TestListTags_SortedWithSnapshotInfo(t *testing.T) {
	...
	dir := t.TempDir()
	snapID := putTestSnapshot(t, st)

	if err := AddTag(context.Background(), st, dir, "v2", snapID); err != nil {
		t.Fatalf("AddTag v2: %v", err)
	}
	if err := AddTag(context.Background(), st, dir, "v1", snapID); err != nil {
		t.Fatalf("AddTag v1: %v", err)
	}

	tags, err := ListTags(context.Background(), st)
	if err != nil {
...
func TestAddTag_Success(t *testing.T) {
	...
	ref, err := st.Refs.GetRef(context.Background(), "tags/v1")
	if err != nil {
...
func TestAddTag_AlreadyExists(t *testing.T) {
	...
	err := AddTag(context.Background(), st, dir, "v1", snapID)
	if !errors.Is(err, errs.ErrTagAlreadyExists) {
		t.Fatalf("expected errs.ErrTagAlreadyExists, got %v", err)
	}
}

// TestAddTag_SnapshotNotFound verifies that adding a tag pointing to a
// non-existent snapshot returns errs.ErrSnapshotNotFound.
func TestAddTag_SnapshotNotFound(t *testing.T) {
	store := setupTestStore(t)
	dir := t.TempDir()
	missingID := core.SnapshotID{Hash: core.Hash{0x99}}
	err := AddTag(context.Background(), st, dir, "v1", missingID)
	if !errors.Is(err, errs.ErrSnapshotNotFound) {
		t.Fatalf("expected errs.ErrSnapshotNotFound, got %v", err)
	}
}
...
func TestAddTag_NFCEquivalent(t *testing.T) {
	...
	if err := AddTag(context.Background(), st, dir, nfc, snapID); err != nil {
		t.Fatalf("AddTag nfc: %v", err)
	}
	// Adding the NFD form should be treated as a duplicate after NFC
	// normalization.
	err := AddTag(context.Background(), st, dir, nfd, snapID)
	if !errors.Is(err, errs.ErrTagAlreadyExists) {
		t.Fatalf("expected errs.ErrTagAlreadyExists for NFD equivalent, got %v", err)
	}
}
...
func TestDeleteTag_Success(t *testing.T) {
	...
	if err := DeleteTag(context.Background(), st, dir, "v1"); err != nil {
		t.Fatalf("DeleteTag: %v", err)
	}
	if _, err := st.Refs.GetRef(context.Background(), "tags/v1"); err == nil {
...
func TestDeleteTag_NotFound(t *testing.T) {
	store := setupTestStore(t)
	dir := t.TempDir()
	err := DeleteTag(context.Background(), st, dir, "v1")
	if !errors.Is(err, errs.ErrTagNotFound) {
		t.Fatalf("expected errs.ErrTagNotFound, got %v", err)
	}
}
...
func TestRenameTag_Success(t *testing.T) {
	...
	if err := RenameTag(context.Background(), st, dir, "v1", "v2"); err != nil {
		t.Fatalf("RenameTag: %v", err)
	}
	if _, err := st.Refs.GetRef(context.Background(), "tags/v1"); err == nil {
		t.Error("expected old tag to be gone")
	}
	ref, err := st.Refs.GetRef(context.Background(), "tags/v2")
	if err != nil {
...
func TestRenameTag_OldNotFound(t *testing.T) {
	store := setupTestStore(t)
	dir := t.TempDir()
	err := RenameTag(context.Background(), st, dir, "v1", "v2")
	if !errors.Is(err, errs.ErrTagNotFound) {
		t.Fatalf("expected errs.ErrTagNotFound, got %v", err)
	}
}
...
// returns errs.ErrTagAlreadyExists.
...
func TestAddTag_ZeroHashRejected(t *testing.T) {
	...
	err := AddTag(context.Background(), st, dir, "v1.0", core.SnapshotID{})
	if err == nil {
	...
	if _, err := st.Refs.GetRef(context.Background(), "tags/v1.0"); err == nil {
...
func TestAddTag_EmptyNameRejected(t *testing.T) {
	...
	err := AddTag(context.Background(), st, dir, "", snapID)
	if err == nil {
...
func TestAddTag_InvalidNameRejected(t *testing.T) {
	...
	for _, name := range invalid {
		t.Run(name, func(t *testing.T) {
			err := AddTag(context.Background(), st, dir, name, snapID)
			if err == nil {
...
func TestAddTag_DoesNotMutateStoreOnFailure(t *testing.T) {
	...
	_ = AddTag(context.Background(), st, dir, "v1.0", core.SnapshotID{})
	_ = AddTag(context.Background(), st, dir, "v1..bad", core.SnapshotID{Hash: core.Hash{0x01}})

	refs, err := st.Refs.ListRefs(context.Background(), "tags/")
	if err != nil {
...
// error (errs.ErrTagAlreadyExists if it ran after the winner released the lock, or
// project.ErrLocked if it contended on the lock while the winner held it). Either way
// no double-create / overwrite occurs.
func TestAddTag_ConcurrentSameName(t *testing.T) {
	store := setupTestStore(t)
	dir := t.TempDir()

	snap1 := putTestSnapshot(t, st)
	snap2 := &core.Snapshot{
	...
	if err := st.Snapshots.PutSnapshot(context.Background(), snap2); err != nil {
	...
	go func() {
		defer wg.Done()
		<-start
		err1 = AddTag(context.Background(), st, dir, "v1", snap1)
	}()
	go func() {
		defer wg.Done()
		<-start
		err2 = AddTag(context.Background(), st, dir, "v1", snap2ID)
	}()
	close(start)
	wg.Wait()
	...
	if !errors.Is(loserErr, errs.ErrTagAlreadyExists) && !errors.Is(loserErr, project.ErrLocked) {
		t.Fatalf("expected loser error to be errs.ErrTagAlreadyExists or project.ErrLocked, got %v", loserErr)
	}
