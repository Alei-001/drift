...
func setupSwitchStore() *store.StoreSet {
...
func TestSwitchBranch_AutoSaveAndRestore(t *testing.T) {
	...
	headRef, err := store.Refs.GetRef(context.Background(), "HEAD")
	if err != nil {
	...
	mainRef, err := store.Refs.GetRef(context.Background(), "heads/main")
	if err != nil {
	...
	featureRef, err := store.Refs.GetRef(context.Background(), "heads/feature")
	if err != nil {
...
func TestSwitchBranch_CreateNew(t *testing.T) {
	...
	expRef, err := store.Refs.GetRef(context.Background(), "heads/experimental")
	if err != nil {
...
func TestSwitchBranch_NoAutosave_CleanWorkspace(t *testing.T) {
	...
	featureRef, _ := store.Refs.GetRef(context.Background(), "heads/feature")
	if featureRef.Target != snap1.ID.Hash {
...
// with uncommitted changes refuses with errs.ErrUncommittedChanges. HEAD must
...
func TestSwitchBranch_NoAutosave_DirtyWorkspace(t *testing.T) {
	...
	if !errors.Is(err, errs.ErrUncommittedChanges) {
		t.Fatalf("expected errs.ErrUncommittedChanges, got %v", err)
	}
	...
	mainRef, _ := store.Refs.GetRef(context.Background(), "heads/main")
	if mainRef.Target != snap1.ID.Hash {
