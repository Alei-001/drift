...
// TestChunkerFor_LargeFile verifies FastCDC is used for large files — the
// deduplication guarantee is never lost, regardless of file size.
func TestChunkerFor_LargeFile(t *testing.T) {
	...
	if _, ok := c.(*chunker.FastCDCChunker); !ok {
		t.Errorf("expected *FastCDCChunker for 600MB, got %T", c)
	}
}
