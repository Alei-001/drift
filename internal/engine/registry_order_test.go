...
import (
	...
	"github.com/Alei-001/drift/internal/engine/binary"
	"github.com/Alei-001/drift/internal/engine/image"
	"github.com/Alei-001/drift/internal/engine/text"
	"github.com/Alei-001/drift/internal/engine/video"
)

// Compile-time assertions that every engine satisfies the engine.Engine
...
var (
	_ engine.Engine = (*text.TextEngine)(nil)
	_ engine.Engine = (*image.ImageEngine)(nil)
	_ engine.Engine = (*video.VideoEngine)(nil)
	_ engine.Engine = (*binary.BinaryEngine)(nil)
)
...
func TestRegistrationOrder(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			e := engine.DetectEngine(tc.path, tc.header)
			if e == nil {
