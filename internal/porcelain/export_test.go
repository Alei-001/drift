...
func exportTestSnapshot(t *testing.T, store *memory.MemoryStorage, tag byte) core.SnapshotID {
	t.Helper()
...
func TestExportSnapshot_NormalExport(t *testing.T) {
	...
	snapID := exportTestSnapshot(t, store, 0x01)

	outPath := filepath.Join(t.TempDir(), "export.zip")
	result, err := ExportSnapshot(context.Background(), store, snapID, outPath)
	if err != nil {
...
func TestExportSnapshot_NonExistentSnapshot(t *testing.T) {
	...
	outPath := filepath.Join(t.TempDir(), "export.zip")
	_, err := ExportSnapshot(context.Background(), store, missingID, outPath)
	if err == nil {
	...
	if !errors.Is(err, storage.ErrNotFound) {
		...
		t.Errorf("expected error to wrap storage.ErrNotFound, got %v", err)
	}
}
...
func TestExportSnapshot_OutputPathCreated(t *testing.T) {
	store := memory.NewMemoryStorage()
	snapID := exportTestSnapshot(t, store, 0x02)
...
func TestExportSnapshot_OverwriteExisting(t *testing.T) {
	store := memory.NewMemoryStorage()
	snapID := exportTestSnapshot(t, store, 0x03)
...
func TestExportSnapshot_EmptySnapshot(t *testing.T) {
	...
	result, err := ExportSnapshot(context.Background(), store, core.SnapshotID{Hash: snapHash}, outPath)
	if err != nil {
		t.Fatalf("ExportSnapshot failed: %v", err)
	}
