...
func importTestBranch(t *testing.T) (*memory.MemoryStorage, []byte, []byte) {
	t.Helper()
	...
	store.PutChunk(context.Background(), cText)
	store.PutChunk(context.Background(), cBin)
...
// does not exist in the branch's snapshot returns ErrFileNotFound.
...
func TestImportFileFromBranch_BranchNoSnapshots(t *testing.T) {
	...
	dir := t.TempDir()
	_, err := ImportFileFromBranch(context.Background(), store, dir, "empty", "file.txt", nil)
	if err == nil {
